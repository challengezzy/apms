package com.apms.flex.modules.apu.correct
{
	...
	/**
	 * APU报文中，错误修正的小时循环进行恢复
	 * @author zzy
	 * @date Jun 14, 2011
	 */
	public class ApuReportHourCycRecoverListener implements CardButtonListener
	{
		...
		private var corFlag:String = "0";//修正恢复
		
		private var corObject:Object = new Object();
		
		public function ApuReportHourCycRecoverListener()
		{
		...
		public function buttonClick(cardPanel:BillCardPanel):void{
			...
			if(endTime == null || endTime == ""){
				SmartXMessage.show("未设置恢复结束时间，确认小时循环从开始时间恢复到现在？",SmartXMessage.MESSAGE_CONFIRM,null,billCardPanel,confirmHandler);
			}else{
				OperatingTipUtil.startOperat("执行小时循环修正恢复...",billCardPanel);
		...
		private function correctOkHandler(event:ResultEvent):void{
			...
			SmartXMessage.show("当前APU时间区间的小时循环数据修正恢复成功！");			
		}
		
		private function faultHandler(event:FaultEvent):void{
			OperatingTipUtil.endOperat();
			SmartXMessage.show("APU小时循环恢复命令执行异常",SmartXMessage.MESSAGE_ERROR,event.fault.faultString);
		}
