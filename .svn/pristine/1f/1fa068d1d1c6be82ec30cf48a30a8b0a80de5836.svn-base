...
public class EmailService {
	...
	public EmailService(){
		//从配置文件中，获取邮件发送配置
		try{
			ApmsSysParamConfService paramService = ApmsSysParamConfService.getInstance();
			EmailConfVo mailConf = paramService.getConfVo().getEmail();
			
			this.host = mailConf.getHost();
			this.username = mailConf.getUser();
			this.password = mailConf.getPassword();
			this.from = mailConf.getSender();
	...
	public static void main(String[] args) throws Exception{
		...
		String msg = "<html><body>APMS系统测试 HTML邮件 + 附件测试</body></html>";
		...
		EmailService mailSrv = new EmailService("smtp.126.com", "apms_system@126.com", "apms_system123", 
				"apms_system@126.com",true);
		//mailSrv.setSessionDebug(true);
		
		mailSrv.sendMail("45766548@qq.com", "邮件测试", msg);
