...
public class ApmsParseTest {
	...
	private static void initDataSources() throws JDOMException, IOException {
		...
		InputStream is = ApmsParseTest.class.getClassLoader().getResourceAsStream(configFileUrl);
		...
		NovaLogger.getLogger(ApmsParseTest.class).debug("[" + str_key + "] = [" + sysRootPath + "]");
			...
			for (int i = 0; i < initparams.size(); i++) {
				if (initparams.get(i) instanceof org.jdom.Element) {
					...
					if (param != null) {
						...
						NovaLogger.getLogger(ApmsParseTest.class).debug("配置参数：" + str_key + " 取值：" + str_value + " 解释：" + str_descr);
						...
						NovaLogger.getLogger(ApmsParseTest.class).debug("[" + str_key + "] = [" + str_value + "]");
					}
				}
			}
		...
		NovaLogger.getLogger(ApmsParseTest.class).debug("开始初始化数据库连接池...");
		if (datasources != null) {
		...
		NovaLogger.getLogger(ApmsParseTest.class).debug("初始化数据库连接池结束,耗时[" + (ll_2 - ll_1) + "]");
	}
	...
	public static void parseTest() throws Exception {
		...
		// 消除已解析数据
		// reparseService.clearComputedReport(ApmsVarConst.RPTNO_A33,"B1877",
		// null, null);

		String begin =   "2017-04-23 23:38:36";
		String end =  "2017-04-25 23:38:36";
		Date beginDate = DateUtil.StringToDate(begin, "yyyy-MM-dd HH:mm:ss");
		Date endDate = DateUtil.StringToDate(end, "yyyy-MM-dd HH:mm:ss");

		String msgno = "438609379";// "";
		String rptno = "A25";
		String acnum = null;// "B6023";
		boolean isOrderby = false; // 36号报文解析要排序

		//reparseService.clearParsedReport(ApmsVarConst.RPTNO_A13, acnum, beginDate, endDate);
		//parseService.parseApuAcarsReport(acnum, begin, end);

		parseService.parseAcarsReport(msgno, rptno, acnum, null, null, isOrderby);// "286494780"
	...
	private static void testApuParse() {
		String apusn = "3745";
		Date beginDate = DateUtil.StringToDate("2017-04-01", "yyyy-MM-dd");

//		String acnum = ApuSwapLogCacheService.getInstance().getAcnumByApuTime(apusn, beginDate);
//		System.out.println(apusn + " ** " + acnum + "  **  " + DateUtil.toString(beginDate));

//		beginDate = DateUtil.StringToDate("2012-04-01", "yyyy-MM-dd");
//		acnum = ApuSwapLogCacheService.getInstance().getAcnumByApuTime(apusn, beginDate);
//		System.out.println(apusn + " ** " + acnum + "  **  " + DateUtil.toString(beginDate));
//
//		beginDate = DateUtil.StringToDate("2014-04-01", "yyyy-MM-dd");
//		acnum = ApuSwapLogCacheService.getInstance().getAcnumByApuTime(apusn, beginDate);
//		System.out.println(apusn + " ** " + acnum + "  **  " + DateUtil.toString(beginDate));
//
//		beginDate = DateUtil.StringToDate("2015-07-01", "yyyy-MM-dd");
//		acnum = ApuSwapLogCacheService.getInstance().getAcnumByApuTime(apusn, beginDate);
//		System.out.println(apusn + " ** " + acnum + "  **  " + DateUtil.toString(beginDate));
//
//		beginDate = DateUtil.StringToDate("2016-11-11", "yyyy-MM-dd");
//		acnum = ApuSwapLogCacheService.getInstance().getAcnumByApuTime(apusn, beginDate);
//		System.out.println(apusn + " ** " + acnum + "  **  " + DateUtil.toString(beginDate));
