 ...
 * 
 */
public class MpfDataParseImpl_A330A340 {
	private Logger logger = NovaLogger.getLogger(this.getClass());
	
	private MpfDataParseImpl_A330A340(){
		
	}
	
	public static MpfDataParseImpl_A330A340 getInstance(){
		MpfDataParseImpl_A330A340 wrn = null;
		if(wrn==null){
			wrn=new MpfDataParseImpl_A330A340();
		}
		return wrn;
	}
...
//							}
					...
					if(secondStr.indexOf("NO WARNING")!=-1){//如果无故障和告警信息，之所以是no warning是因为后面一个词有的是or有的是and
						...
						if(faultAndWrnStr.indexOf("/FR")!=-1){
							String[] faultAndWrnArr=faultAndWrnStr.split("/FR");
							String frs="";//故障信息
							String wns="";//警告信息
							for(int k=0;k<faultAndWrnArr.length;k++){
								if(faultAndWrnArr[k].indexOf("/WN")!=-1){//告警的处理
									String wrnsStr=faultAndWrnArr[k].substring(faultAndWrnArr[k].indexOf("/WN"));
									if(wrnsStr.indexOf(";")==-1){
										String[] wrnArr=wrnsStr.split(",");
										wns=wns+"告警："+wrnArr[0].substring(10)+"\n";
									}else{
										String[] wrnsArr=wrnsStr.split(";");
										for(int n=0;n<wrnsArr.length;n++){
											String[] wrnArr=wrnsArr[n].split(",");
											wns=wns+"告警："+wrnArr[0].substring(10)+"\n";
										}
									}
								}else if(!faultAndWrnArr[k].equals("")){//故障的处理
									if(faultAndWrnArr[k].indexOf(";")==-1){
										String fr1=faultAndWrnArr[k].substring(6);
										String[] frArr1=fr1.split(",");
										frs=frs+"故障："+frArr1[7]+"\n";
									}else{
										String[] frsArr=faultAndWrnArr[k].split(";");
										for(int m=0;m<frsArr.length;m++){
											String fr2=frsArr[m].substring(6);
											String[] frArr2=fr2.split(",");
											frs=frs+"故障："+frArr2[7]+"\n";
										}
									}
								}
						...
						}else if(faultAndWrnStr.indexOf("/WN")!=-1){
							String[] wrnsArr=faultAndWrnStr.split("/WN");
							String wrns="";
							for(int m=0;m<wrnsArr.length;m++){
//								wrns=wrns+"告警："+wrnsArr[m].substring(10)+"\n";
								String wrnsStr2=wrnsArr[m];
								if(!"".equals(wrnsStr2)){
									if(wrnsStr2.indexOf(";")==-1){
										String[] wrnArr=wrnsStr2.split(",");
										wrns=wrns+"告警："+wrnArr[0].substring(10)+"\n";
									}else{
										String[] wrnsArr2=wrnsStr2.split(";");
										for(int n=0;n<wrnsArr2.length;n++){
											String[] wrnArr=wrnsArr2[n].split(",");
											wrns=wrns+"告警："+wrnArr[0].substring(10)+"\n";
										}
									}
								}
