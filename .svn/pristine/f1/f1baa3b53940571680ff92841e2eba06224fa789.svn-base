 ...
 * @date Nov 28, 2011
 **/
public class DTAcarsDataToTempTableDataTask implements DataTaskExecuteIFC {
	...
	public void dataTaskExec(Element task, DataTaskExecThread mainThread)
		...
		mainThread.logTaskRun("开始执行[向Acars临时表抽取数据]任务....");
		try {
			...
			String qrySql="select"+columnStr+"from a_acars_telegraph";
			String insertSql="insert into a_acars_telegraph_temp("+columnStr+") values("+"?,?,?,?,?,?,?,?,?,?,"
			+"?,?,?,?,?,?,?,?,?,?,"+"?,?,?,?,?,?,?,?,?,?,?,?,?)";
			dmo.executeImportByDS(ApmsConst.DS_APMS, qrySql, getFromCols(33), ApmsConst.DS_APMS, insertSql, 4000);
		} catch (Exception e) {
			dmo.rollback(ApmsConst.DS_APMS);
			mainThread.logTaskRun("[向Acars临时表抽取数据]任务执行异常。" + e.getMessage());
		} finally {
		...
		mainThread.logTaskRun("[向Acars临时表抽取数据]任务执行完成！");
