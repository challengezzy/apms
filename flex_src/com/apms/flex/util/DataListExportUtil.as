package com.apms.flex.util
{
	 ...
	 * @date Aug 12, 2011
	 */
	public class DataListExportUtil
	{
		 ...
		 *生成CSV格式文件 逗号"," 分割
		 * */
		public static function copySelectedData(dg:DataGrid,startColumn:int=0, csvSeparator:String=",", lineSeparator:String="\n"):String
		{
			var data:String = "";
			...
			while (!cursor.afterLast)
			{
				...
				if( obj==null || obj.isChecked==null || obj.isChecked != true ){
					continue;
				}
				
				//loop through all columns for the row
				for(var k:int = startColumn; k < columnCount; k++)
				{
					column = columns[k];
					...
					//对单元格内的换行转义,把\r替换成\r\n
					var cellStr:String = StringUtils.StringReplaceAll(column.itemToLabel(obj),"\"","\"\"");
					cellStr = StringUtils.StringReplaceAll(cellStr,"\r","\r\n");
					data += "\""+ cellStr + "\"";
					if(k < (columnCount -1))
					{
						data += csvSeparator;
