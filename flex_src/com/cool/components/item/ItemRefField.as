package com.cool.components.item
{
	...
	import com.cool.components.item.ext.RefDialog;
	import com.cool.components.item.ext.RefInput;
	import com.cool.components.item.ext.RefTreeDialog;
	...
	//参照框，暂不支持任何客户端变量转换{}
	public class ItemRefField extends ItemComponent
	{
		...
		public var formService:RemoteObject = new RemoteObject();
		...
		public var isCondition:Boolean = false;
		...
		public function ItemRefField(){
			...
			inputControl = refInputField;
		}
		
		protected override function initUiControls():void{
		...
		public function set destination(dest:String):void{
			formService.destination = dest;
		}
		
		public function set endpoint(_endpoint:String):void{
			formService.endpoint = _endpoint;
		}
		...
		private function onNovaRefDialogClosed(event:CloseEvent):void{
			if(novaRefDialog != null && novaRefDialog.closeByConfirm){
				...
				*/
				...
				dispatchEvent(new ItemComponentEvent(ItemComponentEvent.REAL_VALUE_CHANGE));
			}
		...
		public override function getQueryConditon(isPreciseQuery:Boolean=false):String{
			if(returnVO != null)
				return " ("+templetItemVO.itemkey+" = '"+returnVO.id+"') ";
			return " (1=1) ";
		}
		...
		public function setRealValue(_value:SimpleRefItemVO):void{
			this.returnVO = _value;
			refInputField.refInput.text = returnVO.name;
			dispatchEvent(new ItemComponentEvent(ItemComponentEvent.REAL_VALUE_CHANGE));
		}
		...
		private function getUnionCondition():String{
			...
			if(unionCondition_Str != null && unionCondition_Str != ""){
				if(controlsArray != null && controlsArray.length>0){
					...
					for each(var itemComponent:ItemComponent in controlsArray){
