 ...
 * 
 */
public class FlrDataParseImpl {
	private Logger logger = NovaLogger.getLogger(this.getClass());
	
	private FlrDataParseImpl(){
		
	}
	
	public static FlrDataParseImpl getInstance(){
		FlrDataParseImpl wrn = null;
		if(wrn==null){
			wrn=new FlrDataParseImpl();
		}
		return wrn;
	}
...
//				}
				...
				int fault_sourceIndex=line.indexOf("/ID");
				if(fault_sourceIndex==-1){
					flrvo.setFault_source(null);
					flrvo.setFault_content(line.substring(20,line.length()));//故障内容
				}else{
					String fault_source=line.substring(fault_sourceIndex+3,line.length());
					flrvo.setFault_source(fault_source);//故障源
					
					String fault_content=line.substring(20,fault_sourceIndex);
					flrvo.setFault_content(fault_content);//故障内容
				}
